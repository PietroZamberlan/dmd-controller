# Build script for dmd_control_closedloop_generate (64-bit)
# Adapted from cppalp-backup/.vscode/build_script_closedloop_generate.ps1

# Find Visual Studio installation
$vsInstallPaths = @(
    "C:\Program Files\Microsoft Visual Studio\2022\BuildTools", # Check 64-bit path first
...
Write-Host "========== DMD CONTROL CLOSEDLOOP GENERATE BUILD SCRIPT (64-BIT) ==========" -ForegroundColor Magenta
...
$sourceDir = Join-Path $repoDir "generate"
$includeDir = Join-Path $repoDir "inc"
$libDir = Join-Path $repoDir "lib\x64"
$outputDir = Join-Path $repoDir "bin\x64"
$outputExe = Join-Path $outputDir "dmd_control_closedloop_generate.exe"
...
$tempBatchFile = Join-Path -Path $env:TEMP -ChildPath "build_dmd_generate_temp.bat"

@"
@echo off
echo Setting up 64-bit environment...
call "$vcvarsPath"
echo Compiling dmd_control_closedloop_generate as 64-bit application...
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $dllSource) {
        ...
        Write-Host "Application may not run correctly without the DLL." -ForegroundColor Yellow
    }
